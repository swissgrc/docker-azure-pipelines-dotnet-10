...
FROM ghcr.io/swissgrc/azure-pipelines-git:2.53.0 AS base
...
LABEL org.opencontainers.image.documentation="https://github.com/swissgrc/docker-azure-pipelines-dotnet-10"
...
RUN apt-get update -y && \
  # Install .NET 10
  apt-get install -y --no-install-recommends dotnet-sdk-10.0=${DOTNET_VERSION} && \
  # Clean up
  apt-get clean && \
  rm -rf /var/lib/apt/lists/* && \
  # Smoke test
